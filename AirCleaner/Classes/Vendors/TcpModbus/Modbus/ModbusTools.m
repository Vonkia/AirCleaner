...
+ (NSString *)calculateModbusRTUStringWithDeviceAddress:(NSString *)deviceAddress
										...
										functionAddress:(NSInteger)functionAddress
										  registerCount:(NSInteger)registerCount
												   data:(NSNumber *)data {
	NSString *commandHexStr = [ModbusTools calculateMultiRegistersModbusRTUStringWithDeviceAddress:deviceAddress functionCode:functionCode functionAddress:functionAddress registerCount:registerCount byteCount:0 data:[NSArray arrayWithObjects:data, nil]];
...
+ (NSData *)calculateModbusRTUBytesWithDeviceAddress:(NSString *)deviceAddress
									 ...
									 functionAddress:(NSInteger)functionAddress
									   registerCount:(NSInteger)registerCount
												data:(NSNumber *)data {
	NSString *commandHexStr = [ModbusTools calculateModbusRTUStringWithDeviceAddress:deviceAddress functionCode:functionCode functionAddress:functionAddress registerCount:registerCount data:data];
	NSData *commandBytes = [ModbusTools convertHexToDataBytes:commandHexStr];
	return commandBytes;
}
...
+ (NSString *)calculateMultiRegistersModbusRTUStringWithDeviceAddress:(NSString *)deviceAddress
	...
	// 计算起始地址的十六进制
	NSString *functionAddressHex = [ModbusTools convertDecimalToHex:functionAddress];
	...
	if (registerCount > 0) {	// 寄存器数量大于0
		registerCountHex = [ModbusTools convertDecimalToHex:registerCount];
	...
	if (byteCount > 0) {	// 字节数大于0
		byteCountHex = [ModbusTools convertDecimalToHex:byteCount];
		// 补齐寄存器数量的十六进制位数到4位，前面补0
	...
	if (data.count > 0) {
		for (NSUInteger i = 0; i < data.count; i ++) {
			NSString *singleDataHex = [ModbusTools convertDecimalToHex:[data[i] longLongValue]];
			// 补齐数据的十六进制位数到4位，前面补0
	...
	uint8_t *hexArray = [ModbusTools convertHexToInt8Array:hexStr];
...
+ (NSData *)calculateMultiRegistersModbusRTUBytesWithDeviceAddress:(NSString *)deviceAddress
												   ...
												   functionAddress:(NSInteger)functionAddress
													 registerCount:(NSInteger)registerCount
														 byteCount:(NSInteger)byteCount
															  data:(NSArray<NSNumber *> *)data {
	NSString *commandHexStr = [ModbusTools calculateMultiRegistersModbusRTUStringWithDeviceAddress:deviceAddress functionCode:functionCode functionAddress:functionAddress registerCount:registerCount byteCount:byteCount data:data];
	NSData *commandBytes = [ModbusTools convertHexToDataBytes:commandHexStr];
	return commandBytes;
}
...
+ (NSArray<NSString *> *)calculateReadResponseModbusDataHexList:(NSString *)responseModbus {
	if ([ModbusTools isResponseModbusError:responseModbus] == NO) {
...
+ (NSArray<NSString *> *)calculateReadResponseModbusDataDecimalList:(NSString *)responseModbus {
	if ([ModbusTools isResponseModbusError:responseModbus] == NO) {
		NSArray *dataHexList = [ModbusTools calculateReadResponseModbusDataHexList:responseModbus];
		NSMutableArray *dataDecimalStrMA = [NSMutableArray array];
		for (NSString *dataHex in dataHexList) {
			unsigned long long dataDecimal = [ModbusTools convertHexToDecimal:dataHex];
...
+ (NSArray<NSString *> *)calculateReadResponseModbusDataBinaryList:(NSString *)responseModbus {
	if ([ModbusTools isResponseModbusError:responseModbus] == NO) {
		...
		NSMutableArray *dataDecimalStrMA = [NSMutableArray array];
		for (NSString *dataHex in dataHexList) {
			NSString *dataBinary = [ModbusTools convertHexToBinary:dataHex];
...
+ (NSString *)calculateReadResponseModbusDataDoubleWord:(NSString *)responseModbus {
	if ([ModbusTools isResponseModbusError:responseModbus] == NO) {
		NSArray *dataList = [ModbusTools calculateReadResponseModbusDataHexList:responseModbus];
		NSString *dataHex = @"";
...
+ (ModbusExceptionCodes)modbusExceptionCode:(NSString *)responseModbus {
	if ([ModbusTools isResponseModbusError:responseModbus]) {
		NSString *exceptionCodeStr = [responseModbus substringWithRange:NSMakeRange(4, 2)];
		return [ModbusTools convertHexStringToHexInt:exceptionCodeStr];
	}
	return 0x00;
}

+ (NSString *)modbusExceptionName:(NSString *)responseModbus {
	ModbusExceptionCodes exceptionCode = [ModbusTools modbusExceptionCode:responseModbus];
...
#pragma mark - 额外增加
+ (NSArray<NSString *> *)calculateReadResponseModbusData:(NSData *)responseData {
    NSString *responseModbus = [self stringFromData:responseData];
    return [self calculateReadResponseModbusDataDecimalList:responseModbus];
}
